...
{
	public function getSearchResult($query){ //Вывод массива результатов поиска
        $link = $this->dbconnect();
        $blogs_arr = array();
        $query = "%".$query."%";

        $prepared = $link->prepare("SELECT blog_url, blog_userlogin, blog_text, blog_title, blog_date, user_name, user_image FROM `blogs` LEFT JOIN `users` ON blogs.blog_userlogin = users.user_login WHERE blog_title LIKE ? ORDER BY blog_id DESC;");
