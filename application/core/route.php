...
class Route
{
	...
	static function start()
	{
		...
		if ((!empty($search[1])) && (strpos($search[1], 'search-request=') !== false)){
			...
			if((($page_name == "settings") || ($page_name == "creator") || ($page_name == "admin")) && ((!isset($_SESSION['auth'])) || ($_SESSION['auth'] == false))){
			...
			if (($page_name == "admin") && (isset($_SESSION['auth'])) && ($_SESSION['auth'] == true)){
				$link = new mysqli('localhost', 'root', 'admin','bloger_db');
			...
			else if(($page_name = "blog") || ($page_name = "profile") || ($page_name = "search") || ($page_name = "editor")){
				$_SESSION['link'] = $link;
